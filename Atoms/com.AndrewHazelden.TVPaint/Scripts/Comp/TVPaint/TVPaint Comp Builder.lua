...
local TVP = {
	...
	-- Should the Input nodes alternate or stagger their spacing based on color
	staggerMode = 0, -- 0: None, 1: Alternating, 2: Staggered
	staggerOffset = 1,
...
function AskForInput()
	...
	TVP.staggerMode = getPreferenceData("TVPaint.staggerMode", TVP.staggerMode, TVP.verbose)
	TVP.staggerOffset = getPreferenceData("TVPaint.staggerOffset", TVP.staggerOffset, TVP.verbose)
	...
	local win = disp:AddWindow({
		ID = "TVPaint", WindowTitle = "TVPaint Comp Builder", Geometry = {100, 100, 320, 630}, Spacing = 10,
		ui:VGroup{
			...
			ui:HGroup{ ui:Label{Text = "Stagger Mode"}, ui:ComboBox{ID = "StaggerMode"} },
			ui:HGroup{ ID = "StaggerOffsetGroup", ui:Label{Text = "Stagger Offset"}, ui:SpinBox{ID = "StaggerOffset", Minimum = -10, Maximum = 10, Value = TVP.staggerOffset} },
	...
	itm.StaggerMode:AddItems({"None", "Alternating", "Staggered"})
	itm.StaggerMode.CurrentIndex = TVP.staggerMode
	...
	itm.BGColorSource.CurrentIndex = TVP.bgColorSource
		...
		local function updateVisibility()
			...
			itm.StaggerOffsetGroup.Visible = (itm.StaggerMode.CurrentIndex > 0)
		...
		function win.On.StaggerMode.CurrentIndexChanged(ev) updateVisibility() end
	...
	function win.On.OKButton.Clicked(ev)
		...
		TVP.staggerMode = itm.StaggerMode.CurrentIndex
		TVP.staggerOffset = itm.StaggerOffset.Value
	...
	local customColor = itm.BGColorPicker.Color
		...
		setPreferenceData("TVPaint.staggerMode", TVP.staggerMode, TVP.verbose)
		setPreferenceData("TVPaint.staggerOffset", TVP.staggerOffset, TVP.verbose)
...
function Main()
	...
	for i = startLayer, endLayer, stepBy do
		...
		-- Extract the layer data first to check color
		local groupTbl = get(tbl.project.clip.layers, i)
		local colorKey = "none"
		if groupTbl.group then 
			colorKey = string.format("%d,%d,%d", groupTbl.group.red, groupTbl.group.green, groupTbl.group.blue)
		end
	
		-- Calculate stagger offset
		if TVP.staggerMode > 0 and colorKey ~= lastColor then
			if TVP.staggerMode == 1 then staggerStep = (staggerStep == 0) and 1 or 0
			elseif TVP.staggerMode == 2 then staggerStep = staggerStep + 1 end
			lastColor = colorKey
		end
		local currentStagger = staggerStep * TVP.staggerOffset
	
		local lX = (TVP.direction == 0) and (origin_x + 2 + currentStagger) or (origin_x + (offsetX * (i - 1) * dirMod))
		local lY = (TVP.direction == 0) and (origin_y + (offsetY * (i - 1) * dirMod)) or (origin_y + 2 + currentStagger)
	...
	for i = startLayer, endLayer, stepBy do
		...
		local iX = (TVP.direction == 0) and (origin_x + 2 + currentStagger) or (origin_x + (offsetX * (i - 1) * dirMod))
		local iY = (TVP.direction == 0) and (origin_y + (offsetY * (i - 1) * dirMod)) or (origin_y + 2 + currentStagger)
	...
	if TVP.groupByColor then
		...
		if #groupNodes > 1 then
			...
			local smX = (TVP.direction == 0) and (maxX + tier2Offset) or (avgX / #groupNodes)
			local smY = (TVP.direction == 1) and (maxY + tier2Offset) or (avgY / #groupNodes)
	...
	local tier3Offset = TVP.nodeSpacing
	local avgX, avgY = 0, 0
	local maxX, maxY = -1000000, -1000000
	...
	local masterX = (TVP.direction == 0) and (maxX + tier3Offset) or (avgX / #finalNodes)
	local masterY = (TVP.direction == 1) and (maxY + tier3Offset) or (avgY / #finalNodes)
